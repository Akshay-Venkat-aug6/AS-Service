...
const register_hotel = async (req, res) => {
  ...
  // console.log(urls)
  const { name, companyname, address, state, city, pin_code, mobileno, email, password } = req.body;
  const hotelEmail = await HotelDetails.findOne({hotel_email: email});
  
  if(hotelEmail){
    return res.send({message: "Email is Already Registered", isRegistered: false})
  }
  ...
  if(checkHotel){
    if(checkHotel.otp === otp){
      return jwt.sign({id: hotel_id}, privateKey, async (err, token)=>{
        await HotelDetails.update({_id: hotel_id}, { $set: { token: token, otp: null }})
        res.send({isChecked: true, token: token,isBusiness: true}).status(200) 
      });
    }
    else{
      return res.send({checkOtp: 'Otp is Invalid', isChecked: false, isBusiness: false})
    }
  }
}
...
const login = async (req, res) => {
  ...
  if(!hotelEmail){
    return res.send({isValid: false, isBusiness: true, message: 'Email is incorrect'}).status(401)
  }
  const user = await HotelDetails.findByEmailAndPassword(email, password)
  // console.log(user)
  if(user){
    jwt.sign({id: user._id}, privateKey, async (err, token)=>{
      await HotelDetails.update({hotel_email: email}, { $set: { token: token }})
      return res.send({token: token, isValid: true, isBusiness: true}).status(200)
    });
  }
  else{
    return res.send({isValid: false, isBusiness: true, message: 'Credentials are incorrect'}).status(401)
  }
};
...
const logout = async (req, res) => {
  ...
  await HotelDetails.updateOne({_id: hotelUser._id}, { $set: { token: null }})
