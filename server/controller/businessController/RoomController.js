...
const createRoom = async(req, res, next) => {
  ...
  if(hotelDetails.price === undefined){
    await HotelDetails.update({_id: hotelDetails._id}, { $set: { price: req.body.price }})
  }
  else{
  ...
  if(hotelDetails.amenities.length === 0 ){
    await HotelDetails.update({_id: hotelDetails._id}, { $push: { amenities:  { $each: [ req.body.amentites[0], req.body.amentites[1], req.body.amentites[2], req.body.amentites[3], req.body.amentites[4], req.body.amentites[5] ] }} })
  }
